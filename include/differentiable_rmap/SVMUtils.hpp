                    ...
                    const Eigen::Matrix<double, inputDim<SamplingSpaceType>(), Eigen::Dynamic> & svm_sv_mat)
{
  ...
  {
    mc_rtc::log::error_and_throw<std::runtime_error>("[calcSVMValue] Only one-class or nu-svc SVM is supported: {}",
                                                     svm_mo->param.svm_type);
  }

  if(svm_param.kernel_type != RBF)
  {
    mc_rtc::log::error_and_throw<std::runtime_error>("[calcSVMValue] Only RBF kernel is supported: {}",
                                                     svm_param.kernel_type);
  }

  return svm_coeff_vec.dot((-svm_param.gamma
                            * (svm_sv_mat.colwise() - sampleToInput<SamplingSpaceType>(sample)).colwise().squaredNorm())
                               .array()
                               .exp()
                               .matrix())
         - svm_mo->rho[0];
    ...
    const Eigen::Matrix<double, inputDim<SamplingSpaceType>(), Eigen::Dynamic> & svm_sv_mat)
{
  if(!(svm_mo->param.svm_type == ONE_CLASS || svm_mo->param.svm_type == NU_SVC))
  {
    mc_rtc::log::error_and_throw<std::runtime_error>("[calcSVMGrad] Only one-class or nu-svc SVM is supported: {}",
                                                     svm_mo->param.svm_type);
  }

  if(svm_param.kernel_type != RBF)
  {
    mc_rtc::log::error_and_throw<std::runtime_error>("[calcSVMGrad] Only RBF kernel is supported: {}",
                                                     svm_param.kernel_type);
  }
  ...
  return inputToSampleMat<SamplingSpaceType>(sample) * 2 * svm_param.gamma * sv_mat_minus_input
         * svm_coeff_vec.cwiseProduct(
             (-svm_param.gamma * sv_mat_minus_input.colwise().squaredNorm()).array().exp().matrix().transpose());
}
    ...
    const Sample<SamplingSpace::SE2> & sample)
{
  ...
  mat.block<sampleDim<SamplingSpace::SO2>(), inputDim<SamplingSpace::SO2>()>(sampleDim<SamplingSpace::R2>(),
                                                                             inputDim<SamplingSpace::R2>()) =
    ...
    const Sample<SamplingSpace::SO3> & sample)
{
  ...
  sample_to_input_mat << 0, -2 * qy, -2 * qz, 0, qy, qx, -qw, -qz, qz, qw, qx, qy, qy, qx, qw, qz, -2 * qx, 0, -2 * qz,
      0, -qw, qz, qy, -qx, qz, -qw, qx, -qy, qw, qz, qy, qx, -2 * qx, -2 * qy, 0, 0;
    ...
    const Sample<SamplingSpace::SE3> & sample)
{
  ...
  mat.block<sampleDim<SamplingSpace::SO3>(), inputDim<SamplingSpace::SO3>()>(sampleDim<SamplingSpace::R3>(),
                                                                             inputDim<SamplingSpace::R3>()) =
...
template<>
inline SampleToVelMat<SamplingSpace::SO3> sampleToVelMat<SamplingSpace::SO3>(const Sample<SamplingSpace::SO3> & sample)
{
  double qw = sample.w();
  double qx = sample.x();
  double qy = sample.y();
  ...
  // Element order of SO3 sample is (x, y, z, w)
  vel_to_sample_mat << qw, -qz, qy, qz, qw, -qx, -qy, qx, qw, -qx, -qy, -qz;

  return vel_to_sample_mat.transpose() / 2;
}

template<>
inline SampleToVelMat<SamplingSpace::SE3> sampleToVelMat<SamplingSpace::SE3>(const Sample<SamplingSpace::SE3> & sample)
{
  ...
  mat.block<velDim<SamplingSpace::SO3>(), sampleDim<SamplingSpace::SO3>()>(velDim<SamplingSpace::R3>(),
                                                                           sampleDim<SamplingSpace::R3>()) =
...
template<>
inline Sample<SamplingSpace::SO3> relSample<SamplingSpace::SO3>(const Sample<SamplingSpace::SO3> & pre_sample,
                                                                const Sample<SamplingSpace::SO3> & suc_sample)
{
  ...
  return (Eigen::Quaterniond(pre_sample.w(), pre_sample.x(), pre_sample.y(), pre_sample.z()).conjugate()
          * Eigen::Quaterniond(suc_sample.w(), suc_sample.x(), suc_sample.y(), suc_sample.z()))
      .coeffs();
}

template<>
inline Sample<SamplingSpace::SE3> relSample<SamplingSpace::SE3>(const Sample<SamplingSpace::SE3> & pre_sample,
                                                                const Sample<SamplingSpace::SE3> & suc_sample)
{
  Sample<SamplingSpace::SE3> sample;
  sample << Eigen::Quaterniond(pre_sample.tail<4>().w(), pre_sample.tail<4>().x(), pre_sample.tail<4>().y(),
                               pre_sample.tail<4>().z())
                    .conjugate()
                * (suc_sample.head<3>() - pre_sample.head<3>()),
      (Eigen::Quaterniond(pre_sample.tail<4>().w(), pre_sample.tail<4>().x(), pre_sample.tail<4>().y(),
                          pre_sample.tail<4>().z())
           .conjugate()
       * Eigen::Quaterniond(suc_sample.tail<4>().w(), suc_sample.tail<4>().x(), suc_sample.tail<4>().y(),
                            suc_sample.tail<4>().z()))
          .coeffs();
  return sample;
...
template<>
inline Sample<SamplingSpace::SE2> relSample<SamplingSpace::SE2>(const Sample<SamplingSpace::SE2> & pre_sample,
                                                                const Sample<SamplingSpace::SE2> & suc_sample)
{
  ...
  rel_sample << cos * sample_error.x() + sin * sample_error.y(), -sin * sample_error.x() + cos * sample_error.y(),
      sample_error.z();
...
{
  if constexpr(SamplingSpaceType == SamplingSpace::R2 || SamplingSpaceType == SamplingSpace::R3)
  {
  ...
  {
    return poseToSample<SamplingSpaceType>(
        sva::interpolate(sampleToPose<SamplingSpaceType>(sample1), sampleToPose<SamplingSpaceType>(sample2), 0.5));
  }
}
    ...
    bool wrt_suc)
{
  ...
  if(wrt_suc)
  {
    mat << qw1, qz1, -qy1, -qx1, -qz1, qw1, qx1, -qy1, qy1, -qx1, qw1, -qz1, qx1, qy1, qz1, qw1;
  }
  else
  {
    mat << -qw2, -qz2, qy2, qx2, qz2, -qw2, -qx2, qy2, -qy2, qx2, -qw2, qz2, qx2, qy2, qz2, qw2;
  }
    ...
    bool wrt_suc)
{
  ...
  mat.block<sampleDim<SamplingSpace::R3>(), sampleDim<SamplingSpace::R3>()>(0, 0) =
      ...
      * Eigen::Quaterniond(pre_sample.tail<4>().w(), pre_sample.tail<4>().x(), pre_sample.tail<4>().y(),
            ...
            .toRotationMatrix()
            .transpose();
  mat.block<sampleDim<SamplingSpace::SO3>(), sampleDim<SamplingSpace::SO3>()>(sampleDim<SamplingSpace::R3>(),
                                                                              sampleDim<SamplingSpace::R3>()) =
  ...
  if(!wrt_suc)
  {
    ...
    for(int j = 0; j < 3; j++)
    {
      for(int k = 0; k < 3; k++)
      {
        mat.block<sampleDim<SamplingSpace::R3>(), sampleDim<SamplingSpace::SO3>()>(0, sampleDim<SamplingSpace::R3>())
            .row(j) += rel_pos[k] * quat_mat.row(j + k * 3);
      }
    }
  }
