...
inline Limb strToLimb(const std::string & limb_str)
{
  ...
  {
    mc_rtc::log::error_and_throw<std::runtime_error>("[strToLimb] Unsupported Limb name: {}", limb_str);
  }
}
} // namespace DiffRmap
...
inline string to_string(Limb limb)
{
  ...
  {
    mc_rtc::log::error_and_throw<std::runtime_error>("[to_string] Unsupported Limb: {}", static_cast<int>(limb));
  }
}
} // namespace std
